...
await sharp(sourceLogo).resize(512, 512).png().toFile("assets/app-icon-512.png");
...
for (const name of screenshots) {
  ...
  await sharp(input).png({ compressionLevel: 9 }).toFile(`assets/screenshots/${name}`);
}
...
const background = Buffer.from(`<svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="94" y="315" fill="#f6c45a" font-family="Arial, sans-serif" font-size="31" font-weight="700">PLAY · LEARN · ENGAGE · COLLECT X-COINS</text>
...
await sharp(background).resize(1024, 538, { fit: "cover" }).extract({ left: 0, top: 19, width: 1024, height: 500 }).composite([{ input: await sharp(sourceLogo).resize(210, 210).png().toBuffer(), left: 710, top: 145 }]).png().toFile("assets/feature-graphic-1024x500.png");
