import { ArrowRight, BadgeCheck, BookOpen, Clapperboard, Gamepad2, Gift, Handshake, ListChecks, LockKeyhole, ShieldCheck, Smartphone, WalletCards } from "lucide-react";
import { Link } from "react-router-dom";
import Seo from "../components/Seo";

const features = [
  { icon: Clapperboard, title: "Watch short videos", text: "Browse reels and short-form content subject to our Community Guidelines." },
  { icon: Gamepad2, title: "Games & Spin", text: "Enjoy games and promotional Spin & Win activities with displayed attempt and reward rules." },
  { icon: ListChecks, title: "Daily tasks", text: "Choose eligible Play & Earn activities and daily tasks with clear completion terms." },
  ...
  { icon: WalletCards, title: "Track X-Coins", text: "Review promotional X-Coin credits, adjustments, wallet balance and reward history." },
  { icon: Gift, title: "Request a reward", text: "Convert or redeem eligible X-Coins for available promotional rewards, subject to review and limits." },
];

const steps = [
  ["01", "Create your account", "Register securely and review the platform rules."],
  ["02", "Choose an activity", "Review requirements before starting a game, spin, daily task or optional affiliate offer."],
...
export default function Home() {
  ...
  return <main id="main-content" className="home-main">
    <Seo title="Future-X | Reels, Games, Tasks & X-Coin Rewards" description="Future-X is an entertainment and engagement platform for reels, games, daily tasks, optional affiliate campaigns and promotional X-Coin rewards." schema={schema} />

    <section className="hero" id="home">
      <div className="hero-grid container">
        <div className="hero-copy">
          <span className="eyebrow"><BadgeCheck size={15} /> Built for responsible rewards</span>
          <h1>Watch. Play. Engage. <span>Collect X-Coins.</span></h1>
          <p>Future-X brings short videos, games, Spin & Win, daily tasks and optional affiliate campaigns into one transparent platform. X-Coins are promotional points—not money or guaranteed income.</p>
        ...
        <div className="hero-device" aria-label="Future-X app preview">
          ...
          <div className="device-note"><Smartphone /><div><strong>Android app</strong><span>Official distribution only</span></div></div>
        </div>
      </div>
    </section>
    ...
    <section className="section container" id="app-preview"><div className="section-heading"><span className="eyebrow">App preview</span><h2>Designed for quick, informed choices</h2><p>Balances, activity rules, reward history and request status stay easy to find.</p></div><div className="screenshots">{previews.map((name, index) => <figure key={name}><img src={`/screenshots/${name}`} alt={`Future-X app screen ${index + 1}`} width="300" height="650" loading={index === 0 ? "eager" : "lazy"} /><figcaption>{["Home", "Games", "Spin & Win", "X-Coin wallet"][index]}</figcaption></figure>)}</div></section>
    ...
    <section className="section faq-preview"><div className="container"><div className="section-heading"><span className="eyebrow">Common questions</span><h2>Know the rules before you begin</h2></div><div className="faq-grid"><article><h3>Are rewards guaranteed?</h3><p>No. X-Coins and redemption requests depend on eligibility, verification, limits, review and availability.</p></article><article><h3>Are X-Coins money?</h3><p>No. They are promotional platform points, not cash, cryptocurrency, a bank balance or income.</p></article><article><h3>Are affiliate offers required?</h3><p>No. Affiliate offers are optional and third-party provider terms apply.</p></article></div><Link className="button button-secondary" to="/faq">View all FAQs</Link></div></section>

    <section className="download-section" id="download"><div className="container download-inner"><div><span className="eyebrow">Android app</span><h2>Use the official Future-X listing</h2><p>For your security, install or update Future-X only through its official Google Play listing when it is available to your account or region. Availability remains subject to Google Play review and distribution settings.</p></div><div className="download-status"><Smartphone /><div><strong>Google Play</strong><span>Availability may vary</span></div></div></div></section>
  </main>;
}
